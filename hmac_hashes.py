...
sha512_blocksize = sha512().block_size
...
def hmac_sha512(key,msg):
	if len(key) > sha512_blocksize:
		key = sha512(key).digest()
	key += chr(0)* (sha512_blocksize - len(key))
	o_key_pad = key.translate(trans_5C)
	i_key_pad = key.translate(trans_36)
	return sha512(o_key_pad+sha512(i_key_pad+msg).digest()).hexdigest()
...
if __name__ == "__main__":
	...
	print "SHA512: "+hmac_sha512("key", "The quick brown fox jumps over the lazy dog")
